...
// Complete M32 netlist versus the unchanged reserved-mode recovery RTL workload.
`ifndef FRP_M32_RESERVED_MODE_RECOVERY_POST_SYNTHESIS_TB_SV
`define FRP_M32_RESERVED_MODE_RECOVERY_POST_SYNTHESIS_TB_SV
`include "frp_m32_reserved_mode_recovery_tb.sv"

module frp_m32_reserved_mode_recovery_post_synthesis_tb;
  timeunit 1ns;
  timeprecision 1fs;
  ...
  // Equality is checked for this workload and parameter set only.
  // This is not model v0.9.3 qualification or global coverage closure.
  frp_m32_reserved_mode_recovery_tb reference_test();
  ...
  int unsigned state_hits[8];
  int unsigned invalid_ticks = 0, invalid_holds = 0, invalid_loads = 0;
  int unsigned invalid_dynamic_ticks = 0, invalid_phase_updates = 0;
  int unsigned invalid_frequency_updates = 0, invalid_capture_ticks = 0;
  int unsigned invalid_auto_ticks = 0, invalid_external_ticks = 0;
  int unsigned invalid_retention_checks = 0, invalid_streak = 0;
  int unsigned longest_invalid_streak = 0, reserved_entries = 0, valid_recoveries = 0;
  logic [1:0] expected_mode = 0;
  int unsigned expected_position = 0;
  logic [31:0] expected_ticks = 0, expected_counts[5];
  ...
  task automatic compare_settled;
    ...
    logic [2:0] old_cadence;
    logic [31:0] counted_ticks;
    ...
    if (armed) begin
      if ($isunknown({reference_outputs, netlist_outputs}) ||
        ...
        mismatch_seen = 1;
        $fatal(1, "M32 reserved-mode recovery post-synthesis mismatch time=%0t step=%0d difference=%h reference=%h netlist=%h",
          $time, reference_test.steps, reference_outputs ^ netlist_outputs,
          reference_outputs, netlist_outputs);
      end
      // Count the old registered cadence, including unmatched INVALID ticks.
      // Clear removes history before this edge's tick; it does not reset cadence.
      old_cadence = cadence(expected_mode, expected_position);
      if (reset_assertion || (rising_edge && !inputs.rst_n)) begin
        expected_mode = 0; expected_position = 0; expected_ticks = 0;
        foreach (expected_counts[i]) expected_counts[i] = 0;
        ...
        if (inputs.tick_enable) begin
          expected_ticks++;
          if (old_cadence < 5) expected_counts[old_cadence]++;
          expected_position = (expected_position+1)%8;
        end
        expected_mode = inputs.scheduler_mode;
      end
      counted_ticks = 0;
      foreach (expected_counts[i]) counted_ticks += expected_counts[i];
      if (netlist_outputs.scheduler_mode_q !== expected_mode ||
          netlist_outputs.scheduler_state_q !== cadence(expected_mode, expected_position) ||
          netlist_outputs.ticks_recorded_q !== expected_ticks ||
          netlist_outputs.scheduler_count_free_q !== expected_counts[0] ||
          netlist_outputs.scheduler_count_balance_q !== expected_counts[1] ||
          netlist_outputs.scheduler_count_commit_q !== expected_counts[2] ||
          netlist_outputs.scheduler_count_excite_q !== expected_counts[3] ||
          netlist_outputs.scheduler_count_neutralize_q !== expected_counts[4] ||
          netlist_outputs.invariant_flags[1] !==
            (expected_mode != 3 && expected_ticks == counted_ticks)) begin
        mismatch_seen = 1;
        $fatal(1, "M32 netlist reserved-mode cadence or counter-history mismatch");
      end
      comparisons++;
      ...
      if (reset_assertion) begin
        reset_assertions++; dynamic_streak = 0; invalid_streak = 0;
      ...
      if (rising_edge) begin
        if (!inputs.rst_n) reset_clock_edges++;
          ...
          if (previous_outputs.scheduler_mode_q != 3 && inputs.scheduler_mode == 3)
            reserved_entries++;
          if (previous_outputs.scheduler_mode_q == 3 && inputs.scheduler_mode != 3)
            valid_recoveries++;
          // The recovery edge still executes the old INVALID cadence.
          if (old_cadence == 7) begin
            invalid_retention_checks++;
            if (netlist_outputs.state_out !== previous_outputs.state_out ||
                netlist_outputs.pending_route_out !== previous_outputs.pending_route_out) begin
              mismatch_seen = 1;
              $fatal(1, "M32 netlist INVALID edge lost retained state or pending routes");
            end
            if (inputs.phase_load_valid) invalid_loads++;
            if (inputs.tick_enable) begin
              invalid_ticks++; invalid_capture_ticks++; invalid_streak++;
              if (invalid_streak > longest_invalid_streak)
                longest_invalid_streak = invalid_streak;
              if (inputs.auto_target_enable) invalid_auto_ticks++; else invalid_external_ticks++;
              if (netlist_outputs.registered_target_q !== previous_outputs.phase_target_source ||
                  !netlist_outputs.registered_target_valid_q) begin
                mismatch_seen = 1;
                $fatal(1, "M32 netlist INVALID tick lost registered target capture");
              end
              if (!inputs.phase_load_valid) begin
                invalid_dynamic_ticks++;
                if (netlist_outputs.phase_word_q != previous_outputs.phase_word_q)
                  invalid_phase_updates++;
                if (netlist_outputs.frequency_current_q16 != previous_outputs.frequency_current_q16)
                  invalid_frequency_updates++;
              end
            end else begin
              invalid_holds++; invalid_streak = 0;
              if (netlist_outputs.registered_target_q !== previous_outputs.registered_target_q ||
                  netlist_outputs.registered_target_valid_q !== previous_outputs.registered_target_valid_q) begin
                mismatch_seen = 1;
                $fatal(1, "M32 netlist held INVALID edge changed registered target history");
              end
            end
          end else invalid_streak = 0;
          if (inputs.phase_load_valid) begin
          ...
          if (inputs.tick_enable) begin
            ...
            if (old_cadence != previous_outputs.scheduler_state_q ||
                (old_cadence > 4 && old_cadence != 7)) begin
              mismatch_seen = 1;
              $fatal(1, "Unexpected M32 netlist pre-edge scheduler state");
            end
            scheduler_states[old_cadence] = 1; state_hits[old_cadence]++;
  ...
  // Check reset, both clock edges and stopped-clock controls after settling.
  // The unchanged reference checks its independent oracle one ns later.
  initial begin
    if (INPUT_BITS != 1060 || OUTPUT_BITS != 2075)
      $fatal(1, "Incomplete M32 reserved-mode recovery port bundles");
  ...
  final begin
    if (!armed || mismatch_seen || input_changes == 0 || stopped_clock_changes == 0 ||
        ...
        enabled_ticks != 22521 || dynamic_ticks != 22317 || phase_updates != dynamic_ticks ||
        frequency_updates == 0 || hold_edges != 3660 || falling_edges != 27051 ||
        reset_assertions != 870 || reset_clock_edges != 870 || reset_releases != 870 ||
        pending_resets != 48 || clear_tick_edges != 228 || clear_hold_edges != 978 ||
        phase_load_edges != 1236 || automatic_ticks + external_ticks != enabled_ticks ||
        first_legs != 1700 || second_legs != 1604 || first_cells != 8'hff || second_cells != 8'hff ||
        scheduler_states != 8'h9f || target_lag_samples != 6310 ||
        state_feedback_samples != 149768 || switch_feedback_samples != 9812 ||
        heating_ticks != 4926 || cooling_ticks != 12659 ||
        state_hits[0] != 11209 || state_hits[1] != 3668 || state_hits[2] != 524 ||
        state_hits[3] != 524 || state_hits[4] != 3668 || state_hits[5] != 0 || state_hits[6] != 0 ||
        state_hits[7] != 2928 || invalid_ticks != 2928 || invalid_holds != 630 ||
        invalid_loads != 192 || invalid_dynamic_ticks != 2832 || invalid_phase_updates != 2832 ||
        invalid_frequency_updates != 1494 || invalid_capture_ticks != 2928 ||
        invalid_auto_ticks != 1308 || invalid_external_ticks != 1620 ||
        invalid_retention_checks != 3558 || longest_invalid_streak != 256 ||
        reserved_entries != 870 || valid_recoveries != 822)
      $fatal(1, "Incomplete M32 reserved-mode recovery netlist coverage comparisons=%0d inputs=%0d ticks=%0d dynamic=%0d phase=%0d frequency=%0d holds=%0d falls=%0d resets=%0d/%0d/%0d invalid=%0d/%0d entries=%0d recoveries=%0d streak=%0d",
        comparisons, input_changes, enabled_ticks, dynamic_ticks, phase_updates, frequency_updates,
        hold_edges, falling_edges, reset_assertions, reset_clock_edges, reset_releases,
        invalid_ticks, invalid_holds, reserved_entries, valid_recoveries, longest_invalid_streak);
    if (reference_test.profiles != 6 || reference_test.steps != 26181 ||
        reference_test.checks != 86133 || reference_test.live_control_checks != 4110 ||
        reference_test.entry_cases != 384 || reference_test.recovery_cases != 384 ||
        reference_test.reset_cases != 48 || reference_test.stream_ticks != 1536 ||
        reference_test.recovery_retention_checks != 384 || reference_test.drained_route_checks != 774 ||
        reference_test.diagnostic_checks != 4380 || reference_test.retained_diagnostic_checks != 3462 ||
        reference_test.restored_diagnostic_checks != 918 ||
        ...
        reference_test.invalid_ticks != invalid_ticks || reference_test.invalid_holds != invalid_holds ||
        reference_test.invalid_loads != invalid_loads ||
        reference_test.invalid_dynamic_ticks != invalid_dynamic_ticks ||
        reference_test.invalid_phase_updates != invalid_phase_updates ||
        reference_test.invalid_frequency_updates != invalid_frequency_updates ||
        reference_test.invalid_capture_ticks != invalid_capture_ticks ||
        reference_test.invalid_auto_ticks != invalid_auto_ticks ||
        reference_test.invalid_external_ticks != invalid_external_ticks ||
        reference_test.invalid_retention_checks != invalid_retention_checks)
      $fatal(1, "Incomplete M32 reserved-mode recovery reference qualification");
    foreach (state_hits[i])
      if (state_hits[i] != reference_test.state_hits[i])
        $fatal(1, "M32 reserved-mode recovery scheduler-state counts differ");
    foreach (reference_test.entry_seen[a,b,c])
      if (!reference_test.entry_seen[a][b][c] || !reference_test.recovery_seen[a][b][c])
        $fatal(1, "Incomplete M32 reserved-mode entry or recovery matrix");
    foreach (reference_test.reset_seen[a,b])
      if (!reference_test.reset_seen[a][b])
        $fatal(1, "Incomplete M32 reserved-mode reset matrix");
    if ($isunknown({reference_outputs, netlist_outputs}) || reference_outputs !== netlist_outputs)
      $fatal(1, "Final M32 reserved-mode recovery output mismatch");
    $display("FRP_M32_RESERVED_MODE_RECOVERY_POST_SYNTHESIS_TB: PASS comparisons=%0d input_bits=%0d output_bits=%0d input_changes=%0d stopped_clock_changes=%0d enabled_ticks=%0d dynamic_ticks=%0d longest_streak=%0d phase_updates=%0d frequency_updates=%0d hold_edges=%0d falling_edges=%0d reset_assertions=%0d reset_clock_edges=%0d reset_releases=%0d pending_resets=%0d clear_tick_edges=%0d clear_hold_edges=%0d phase_load_edges=%0d automatic_ticks=%0d external_ticks=%0d target_lag_samples=%0d state_feedback_samples=%0d switch_feedback_samples=%0d heating_ticks=%0d cooling_ticks=%0d first_legs=%0d second_legs=%0d first_cells=%0d second_cells=%0d scheduler_states=%0d free_ticks=%0d balance_ticks=%0d commit_ticks=%0d excite_ticks=%0d neutralize_ticks=%0d invalid_ticks=%0d invalid_holds=%0d invalid_loads=%0d invalid_dynamic_ticks=%0d invalid_phase_updates=%0d invalid_frequency_updates=%0d invalid_capture_ticks=%0d invalid_auto_ticks=%0d invalid_external_ticks=%0d invalid_retention_checks=%0d longest_invalid_streak=%0d reserved_entries=%0d valid_recoveries=%0d reference_profiles=%0d reference_steps=%0d reference_checks=%0d reference_live_control_checks=%0d entry_cases=%0d recovery_cases=%0d reset_cases=%0d stream_ticks=%0d diagnostic_checks=%0d retained_diagnostic_checks=%0d restored_diagnostic_checks=%0d recovery_retention_checks=%0d drained_route_checks=%0d",
      ...
      invalid_ticks, invalid_holds, invalid_loads, invalid_dynamic_ticks, invalid_phase_updates,
      invalid_frequency_updates, invalid_capture_ticks, invalid_auto_ticks, invalid_external_ticks,
      invalid_retention_checks, longest_invalid_streak, reserved_entries, valid_recoveries,
      reference_test.profiles, reference_test.steps, reference_test.checks, reference_test.live_control_checks,
      reference_test.entry_cases, reference_test.recovery_cases, reference_test.reset_cases,
      reference_test.stream_ticks, reference_test.diagnostic_checks, reference_test.retained_diagnostic_checks,
      reference_test.restored_diagnostic_checks, reference_test.recovery_retention_checks,
      reference_test.drained_route_checks);
  end
endmodule : frp_m32_reserved_mode_recovery_post_synthesis_tb
`endif
