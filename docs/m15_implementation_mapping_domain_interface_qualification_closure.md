...
1.  
resolve scheduler state for tick;
...
5.  
process external request lanes in ascending lane order;
 
 
6.  
process phase-derived reference targets when `auto_targets_enable = 1`;
 
 
7.  
calculate current tick switch load;
...
9.  
update lagged frequency states;
...
15.  
update deterministic gamma-noise correlation states;
...
21.  
calculate multiscale phase-coherence values;
 
 
22.  
calculate global C(t);
 
 
23.  
calculate global P(t);
...
25.  
detect first positive-to-nonpositive stability crossing;
 
 
26.  
capture post-tick trace outputs.
...
1.  
parse next vector row;
 
 
2.  
drive tick input signals before the active clock edge;
...
5.  
allow deterministic registered outputs to settle;
