...
class FRPCore:
    ...
    def compute(self, op, a, b=None, steps=128, amp=0.30, include_telemetry=False):
...
class FRPProcessor:
    def __init__(
        ...
        **core_kwargs,
        ...
        self.amp = float(amp)
        self.include_telemetry = bool(include_telemetry)
...
def direct_baseline(a, target):
    ...
    return {
        ...
        "prevented_direct_events_total": 0,
...
def run_demo(args):
    ...
    payload = {
        ...
        "version": VERSION,
        "kind": "demo",
        "parameters": command_profile(args),
        "log": log,
    ...
    print("FRP PROCESSOR DEMO v0.9.4")
...
def run_test(args):
                ...
                for op in ops:
                    ...
                    if fail and first_failure is None:
                        ...
                        if args.include_telemetry:
                            first_failure["telemetry"] = r.get("telemetry", [])
    ...
    failure_count = 1 if first_failure else 0
    result = "FAIL" if first_failure else "PASS"
...
def run_bench(args):
        ...
        rows_out.append(
            {
                ...
                "heat_peak": heat,
                "switch_load_peak": sw,
                "actual_direct_events_total": actual,
                "prevented_direct_events_total": prevented,
                "neutralized_conflicts_total": neutralized,
            }
        )
    ...
    payload = {
        ...
        "kind": "benchmark",
        "parameters": command_profile(args),
        "architectures": rows_out,
        "benchmark_supported_position": (
            "FRP adds a Kuramoto-Sakaguchi resonant phase layer on top of "
            "safe distributed neutral ternary transition logic while preserving "
            "zero actual direct -1 <-> 1 transitions in the tested operational domain."
        ),
    }
    ...
    for row in rows_out:
        print(
            ...
            f"{row['cases']} | "
            f"{row['match']:.3f} | "
            f"{row['C_minus_P_min']:.6f} | "
            f"{row['heat_peak']:.6f} | "
            f"{row['switch_load_peak']:.6f} | "
            f"{row['actual_direct_events_total']} | "
            f"{row['prevented_direct_events_total']} | "
            f"{row['neutralized_conflicts_total']}"
        )


def parse_args():
    p = argparse.ArgumentParser(
        description="FRP v0.9.4 single-file prototype with structured output"
    )
    ...
    p.add_argument(
        ...
        choices=["text", "json"],
        default="text",
        help="output format: text preserves v0.9.3-style console output; json emits structured machine-readable output",
    )

    p.add_argument(
        "--include-telemetry",
        action="store_true",
        help="include per-tick telemetry in JSON output where applicable",
    )
