...
// Complete M32 netlist versus the unchanged integration RTL workload.
`ifndef FRP_M32_EXECUTION_INTEGRATION_POST_SYNTHESIS_TB_SV
`define FRP_M32_EXECUTION_INTEGRATION_POST_SYNTHESIS_TB_SV
`include "frp_m32_execution_integration_tb.sv"

module frp_m32_execution_integration_post_synthesis_tb;
  timeunit 1ns;
  timeprecision 1fs;
  ...
  // The reference keeps its independent oracle, stimuli and watchdog.
  frp_m32_execution_integration_tb reference_test();
  ...
  // Hierarchical reads are restricted to the reference DUT's public ports,
  // including outputs left open by the standalone ternary/cadence scoreboard.
  assign inputs = '{
  ...
  task automatic compare_settled;
    ...
    if (armed) begin
      if ($isunknown({reference_outputs, netlist_outputs}) ||
        ...
        mismatch_seen = 1;
        $fatal(1, "M32 integration post-synthesis mismatch time=%0t step=%0d difference=%h reference=%h netlist=%h",
          $time, reference_test.steps, reference_outputs ^ netlist_outputs,
          reference_outputs, netlist_outputs);
      end
      comparisons++;
      if (controls !== previous_controls) input_changes++;
      ...
      if (rising_edge) begin
        if (!inputs.rst_n) reset_clock_edges++;
          ...
          if (inputs.phase_load_valid) phase_load_edges++;
          ...
          if (inputs.tick_enable) begin
            ...
            // Count admitted route legs before the edge and completions after it.
            // A retained opposite destination must execute on a later tick.
  ...
  // One femtosecond allows zero-delay cells to settle before the reference's
  // one-nanosecond checks; asynchronous reset and held-clock changes are covered.
  initial begin
    if (INPUT_BITS != 1060 || OUTPUT_BITS != 2075)
      $fatal(1, "Incomplete M32 port bundles");
  ...
  final begin
    if (!armed || mismatch_seen || input_changes == 0 ||
        comparisons < 2*reference_test.steps + 4*reference_test.resets ||
        enabled_ticks != 22789 || hold_edges != 8635 || falling_edges != 34572 ||
        reset_assertions != 3148 || reset_clock_edges != 3148 || reset_releases != 3148 ||
        clear_tick_edges != 863 || clear_hold_edges != 1208 || phase_load_edges != 31424 ||
        automatic_ticks == 0 || external_ticks == 0 ||
        automatic_ticks + external_ticks != enabled_ticks ||
        first_legs != 1652 || second_legs != 1648 || first_cells != 8'hff ||
        second_cells != 8'hff || scheduler_states != 5'b11111)
      $fatal(1, "Incomplete M32 post-synthesis coverage comparisons=%0d inputs=%0d ticks=%0d holds=%0d falls=%0d resets=%0d/%0d/%0d clears=%0d/%0d loads=%0d routes=%0d/%0d",
        comparisons, input_changes, enabled_ticks, hold_edges, falling_edges,
        reset_assertions, reset_clock_edges, reset_releases, clear_tick_edges,
        clear_hold_edges, phase_load_edges, first_legs, second_legs);
    if (reference_test.matrix_cases != 2880 || reference_test.mask_cases != 256 ||
        reference_test.stream_steps != 3072 || reference_test.steps != 31424 ||
        reference_test.checks != 106864 || reference_test.resets != reset_assertions ||
        reference_test.enabled_ticks != enabled_ticks || reference_test.held_ticks != hold_edges ||
        reference_test.clear_enabled != clear_tick_edges || reference_test.clear_held != clear_hold_edges ||
        reference_test.first_legs != first_legs || reference_test.second_legs != second_legs ||
        reference_test.automatic_cells != 8'hff || reference_test.external_cells != 8'hff ||
        reference_test.first_cells != first_cells || reference_test.second_cells != second_cells ||
        reference_test.scheduler_states != scheduler_states || reference_test.capacity_blocks != 748 ||
        reference_test.mux_changes != 4440 || reference_test.target_lag_samples != 8830)
      $fatal(1, "Incomplete M32 integration reference qualification");
    $display("FRP_M32_EXECUTION_INTEGRATION_POST_SYNTHESIS_TB: PASS comparisons=%0d input_bits=%0d output_bits=%0d input_changes=%0d enabled_ticks=%0d hold_edges=%0d falling_edges=%0d reset_assertions=%0d reset_clock_edges=%0d reset_releases=%0d clear_tick_edges=%0d clear_hold_edges=%0d phase_load_edges=%0d automatic_ticks=%0d external_ticks=%0d first_legs=%0d second_legs=%0d first_cells=%0d second_cells=%0d scheduler_states=%0d matrix_cases=%0d mask_cases=%0d stream_steps=%0d reference_checks=%0d",
      comparisons, INPUT_BITS, OUTPUT_BITS, input_changes, enabled_ticks, hold_edges,
      falling_edges, reset_assertions, reset_clock_edges, reset_releases,
      ...
      reference_test.matrix_cases, reference_test.mask_cases, reference_test.stream_steps,
      reference_test.checks);
  end
endmodule : frp_m32_execution_integration_post_synthesis_tb
`endif
