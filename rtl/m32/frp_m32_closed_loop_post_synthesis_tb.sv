...
// Complete M32 netlist versus the unchanged continuous-feedback RTL workload.
`ifndef FRP_M32_CLOSED_LOOP_POST_SYNTHESIS_TB_SV
`define FRP_M32_CLOSED_LOOP_POST_SYNTHESIS_TB_SV
`include "frp_m32_closed_loop_tb.sv"

module frp_m32_closed_loop_post_synthesis_tb;
  timeunit 1ns;
  timeprecision 1fs;
  ...
  task automatic compare_settled;
    ...
    if (armed) begin
      if ($isunknown({reference_outputs, netlist_outputs}) ||
        ...
        mismatch_seen = 1;
        $fatal(1, "M32 closed-loop post-synthesis mismatch time=%0t step=%0d difference=%h reference=%h netlist=%h",
  ...
  initial begin
    if (INPUT_BITS != 1060 || OUTPUT_BITS != 2075)
      $fatal(1, "Incomplete M32 closed-loop port bundles");
  ...
  final begin
    if (!armed || mismatch_seen || input_changes == 0 || stopped_clock_changes == 0 ||
        ...
        enabled_ticks != 3104 || dynamic_ticks != 3104 || longest_streak != 256 ||
        phase_updates == 0 || frequency_updates == 0 || hold_edges != 158 || falling_edges != 3284 ||
        reset_assertions != 22 || reset_clock_edges != 22 || reset_releases != 22 || pending_resets != 8 ||
        clear_tick_edges != 54 || clear_hold_edges != 86 || phase_load_edges != 12 ||
        automatic_ticks != 2688 || external_ticks != 416 ||
        automatic_ticks + external_ticks != enabled_ticks ||
        first_legs != 185 || second_legs != 177 || first_legs-second_legs != pending_resets ||
        first_cells != 8'hff || second_cells != 8'hff || scheduler_states != 5'b11111 ||
        target_lag_samples != 988 || state_feedback_samples != 14274 ||
        switch_feedback_samples != 1502 || heating_ticks != 1092 || cooling_ticks != 1733 ||
        state_hits[0] != 1056 || state_hits[1] != 896 || state_hits[2] != 128 ||
        state_hits[3] != 128 || state_hits[4] != 896)
      $fatal(1, "Incomplete M32 closed-loop post-synthesis coverage comparisons=%0d inputs=%0d ticks=%0d holds=%0d falls=%0d resets=%0d/%0d/%0d pending_resets=%0d clears=%0d/%0d loads=%0d routes=%0d/%0d feedback=%0d/%0d thermal=%0d/%0d",
        comparisons, input_changes, enabled_ticks, hold_edges, falling_edges,
        reset_assertions, reset_clock_edges, reset_releases, pending_resets,
...
endmodule : frp_m32_closed_loop_post_synthesis_tb
`endif
