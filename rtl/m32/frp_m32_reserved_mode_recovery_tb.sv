...
// Full M32 reserved-mode entry, containment and recovery qualification.
// Mode 11 presents INVALID=7 after capture. Execution retains active-zero
// routes; enabled phase/frequency dynamics and target capture still advance.
// The old registered state governs the handoff edge. Invalid-state ticks
// remain visible as a scheduler-count discrepancy until clear or reset.
// All 59 public outputs are checked by an independent fixed-point oracle
// retained from the closed-loop and mode-handoff regressions. No internal
// DUT state, ROM data or arithmetic/transition helpers are read.
// This is eight-cell RTL qualification, not model v0.9.3 equivalence or
// global coverage closure. No physical power or temperature is measured.
`timescale 1ns/1ps
`include "frp_m32_core.sv"

module frp_m32_reserved_mode_recovery_tb;
  import frp_m31_pkg::*;
  ...
  int state_hits[8];
  int invalid_ticks = 0, invalid_holds = 0, invalid_dynamic_ticks = 0;
  int invalid_phase_updates = 0, invalid_frequency_updates = 0;
  int invalid_capture_ticks = 0, invalid_auto_ticks = 0, invalid_external_ticks = 0;
  int invalid_loads = 0, invalid_retention_checks = 0;
  int diagnostic_checks = 0, retained_diagnostic_checks = 0, restored_diagnostic_checks = 0;
  ...
  task automatic observe;
    ...
    counted_ticks = 0;
    foreach (gold_counts[i]) counted_ticks += gold_counts[i];
    // Bit 1 includes both scheduler validity and the recorded-count relation.
    // Valid-mode recovery alone cannot erase ticks spent in INVALID state.
    expected.invariant_flags[1] = gold_mode != 3 && gold_ticks == counted_ticks;
    // INVALID also disqualifies transition replay, capacity qualification and
    // retained-state update qualification, even when every state is held.
    if (gold_mode == 3) expected.invariant_flags[6:4] = 3'b000;
  ...
  task automatic check;
    ...
    if (actual !== expected) begin
      $display("M32 reserved-mode recovery mismatch step=%0d mode=%0d position=%0d cadence=%0d", steps, gold_mode, gold_position, cadence());
      ...
      $fatal(1, "M32 reserved-mode recovery public output mismatch");
    end
  endtask
  ...
  task automatic step(input bit enabled, input bit clear = 0);
    ...
    #1; check();
    before_edge = actual; was_invalid = gold_mode == 3;
    ...
    if (enabled) begin
      state_hits[cadence()]++;
      if (cadence() < 5) gold_counts[cadence()]++;
      gold_ticks++;
    ...
    clk = 1; #1; steps++; check();
    if (was_invalid) begin
      if (actual.state_out !== before_edge.state_out ||
          actual.pending_route_out !== before_edge.pending_route_out)
        $fatal(1, "INVALID execution changed retained state or pending routes");
      invalid_retention_checks++;
      if (enabled) begin
        invalid_ticks++; invalid_capture_ticks++;
        if (auto_target_enable) invalid_auto_ticks++; else invalid_external_ticks++;
        if (actual.registered_target_q !== before_edge.phase_target_source)
          $fatal(1, "INVALID scheduler stopped or bypassed target capture");
        if (!phase_load_valid) begin
          invalid_dynamic_ticks++;
          if (actual.phase_word_q != before_edge.phase_word_q) invalid_phase_updates++;
          if (actual.frequency_current_q16 != before_edge.frequency_current_q16)
            invalid_frequency_updates++;
        end
      end else begin
        invalid_holds++;
        if (actual.registered_target_q !== before_edge.registered_target_q)
          $fatal(1, "Held INVALID edge changed registered target history");
      end
      if (phase_load_valid) invalid_loads++;
    end
    clk = 0; #1; check();
  endtask
  ...
  bit entry_seen[3][8][16], recovery_seen[3][8][16], reset_seen[3][16];
  int entry_cases = 0, recovery_cases = 0, reset_cases = 0;
  int drained_route_checks = 0, recovery_retention_checks = 0, stream_ticks = 0;
  ...
  task automatic select_mode(input int mode);
    observation_t saved;
    #1; check(); live_control_checks++;
    saved = actual;
    scheduler_mode = frp_m31_scheduler_mode_e'(mode);
    #1; check(); live_control_checks++;
    if (actual !== saved)
      $fatal(1, "Input mode bypassed the clocked scheduler boundary");
  endtask

  task automatic diagnostic(input bit valid);
    if (actual.invariant_flags[1] !== valid)
      $fatal(1, "Incorrect reserved-mode or counter-history diagnostic");
    diagnostic_checks++;
    if (valid) restored_diagnostic_checks++;
    else retained_diagnostic_checks++;
  endtask

  task automatic prepare_routes(input int mode, position, output int first_cell);
    ...
    scheduler_mode = frp_m31_scheduler_mode_e'(mode); step(1);
    if (gold_position != position || gold_mode != mode ||
        actual.state_out[2*first_cell +: 4] != 0 ||
        actual.pending_route_out[2*first_cell +: 4] != 4'b0111)
      $fatal(1, "Missing initial pair of active-zero routes");
    // New requests conflict with the saved destination polarities.
    lane(0, first_cell, 1); lane(1, first_cell+1, -1);
    #1; check(); live_control_checks++;
  endtask

  task automatic controls(input int word);
    auto_target_enable = (word & 8) != 0;
    phase_load_valid = (word & 4) != 0;
    phase_load = ~gold_phase; frequency_load_q16 = ~gold_frequency;
  endtask

  task automatic drain_routes(input int mode, first_cell);
    phase_load_valid = 0; auto_target_enable = 0; external_request_valid = 0;
    scheduler_mode = frp_m31_scheduler_mode_e'(mode); step(0);
    step(0, 1); diagnostic(1);
    repeat (16) step(1);
    if (actual.pending_route_out != 0 || actual.state_out[2*first_cell +: 4] != 4'b0111)
      $fatal(1, "Saved routes did not complete through the recovered cadence");
    drained_route_checks++;
  endtask

  task automatic entry_case(input int old_mode, position, word);
    int first_cell;
    prepare_routes(old_mode, position, first_cell);
    select_mode(3); controls(word); step((word & 1) != 0, (word & 2) != 0);
    diagnostic(0);
    if (actual.scheduler_mode_q != 3 || actual.scheduler_state_q != 7)
      $fatal(1, "Reserved mode was not captured exactly");
    phase_load_valid = 0; step(1); diagnostic(0);
    select_mode(old_mode); step(0); diagnostic(0);
    drain_routes(old_mode, first_cell);
    if (entry_seen[old_mode][position][word]) $fatal(1, "Repeated entry bin");
    entry_seen[old_mode][position][word] = 1; entry_cases++;
  endtask

  task automatic recovery_case(input int mode, position, word);
    int first_cell;
    observation_t before_edge;
    prepare_routes(mode, (position+6)%8, first_cell);
    select_mode(3); step(0);
    step(1); step(1); diagnostic(0);
    if (gold_position != position) $fatal(1, "Recovery cycle alignment differs");
    select_mode(mode); controls(word); before_edge = actual;
    step((word & 1) != 0, (word & 2) != 0);
    if (actual.state_out !== before_edge.state_out ||
        actual.pending_route_out !== before_edge.pending_route_out)
      $fatal(1, "Recovery edge used incoming mode before registration");
    recovery_retention_checks++;
    // A held clear removes old discrepancies. Clear plus an invalid tick
    // records one unmatched tick and must retain the diagnostic afterward.
    diagnostic((word & 3) == 2);
    drain_routes(mode, first_cell);
    if (recovery_seen[mode][position][word]) $fatal(1, "Repeated recovery bin");
    recovery_seen[mode][position][word] = 1; recovery_cases++;
  endtask

  initial begin : qualification
    real sample;
    int first_cell, before_dynamic, before_loads, before_resets;
    ...
    foreach (gold_counts[i]) gold_counts[i] = 0;
    foreach (entry_seen[a,b,c]) entry_seen[a][b][c] = 0;
    foreach (recovery_seen[a,b,c]) recovery_seen[a][b][c] = 0;
    foreach (reset_seen[a,b]) reset_seen[a][b] = 0;
    for (int mode = 0; mode < 3; mode++)
      for (int position = 0; position < 8; position++)
        for (int word = 0; word < 16; word++) begin
          entry_case(mode, position, word);
          recovery_case(mode, position, word);
        end
    foreach (entry_seen[a,b,c])
      if (!entry_seen[a][b][c] || !recovery_seen[a][b][c])
        $fatal(1, "Missing reserved-mode matrix bin");
    $display("FRP_M32_RESERVED_MODE_MATRIX: PASS entry_cases=%0d recovery_cases=%0d recovery_retention_checks=%0d drained_route_checks=%0d",
      entry_cases, recovery_cases, recovery_retention_checks, drained_route_checks);

    // Six uninterrupted INVALID intervals retain pending routes while the
    // phase/frequency feedback and registered target path continue to run.
    for (int workload = 0; workload < 2; workload++)
      for (int mode = 0; mode < 3; mode++) begin
        prepare_routes(mode, mode+workload, first_cell); load_profile(workload);
        select_mode(3); step(0);
        before_dynamic = dynamic_ticks; before_loads = loads; before_resets = resets;
        for (int n = 0; n < 256; n++) begin
          auto_target_enable = n%3 != 0;
          if (workload == 1) begin
          ...
          step(1, n%67 == 0); diagnostic(0); stream_ticks++;
        end
        if (dynamic_ticks-before_dynamic != 256 || loads != before_loads || resets != before_resets)
          $fatal(1, "Reserved-mode stream was interrupted");
        $display("FRP_M32_RESERVED_MODE_STREAM: PASS workload=%0d recovery_mode=%0d ticks=256 phase=%h frequency=%h state=%h pending=%h temperature=%h peak=%h",
          workload, mode, actual.phase_word_q, actual.frequency_current_q16, actual.state_out,
          actual.pending_route_out, actual.temperature_proxy_q16, actual.peak_temperature_proxy_q16);
        select_mode(mode); step(0); diagnostic(0);
        drain_routes(mode, first_cell); profiles++;
      end

    // Asynchronous reset wins over reserved mode and every tick/clear/load/
    // automatic-request combination, including pending routes and stale counts.
    for (int mode = 0; mode < 3; mode++)
      for (int word = 0; word < 16; word++) begin
        prepare_routes(mode, word%8, first_cell);
        select_mode(3); step(0); step(1); diagnostic(0);
        controls(word); tick_enable = (word & 1) != 0; clear_counters = (word & 2) != 0;
        reset_core(1);
        if (actual.scheduler_mode_q != 0 || actual.scheduler_state_q != 0 ||
            actual.state_out != 0 || actual.pending_route_out != 0 ||
            actual.registered_target_valid_q || actual.registered_target_q != 0 ||
            actual.ticks_recorded_q != 0)
          $fatal(1, "Reset did not clear reserved-mode execution history");
        step((word & 1) != 0, (word & 2) != 0); diagnostic(0);
        phase_load_valid = 0; auto_target_enable = 0; external_request_valid = 0;
        select_mode(mode); step(0); diagnostic(1);
        reset_seen[mode][word] = 1; reset_cases++;
      end
    foreach (reset_seen[a,b]) if (!reset_seen[a][b]) $fatal(1, "Missing reset bin");
    if (entry_cases != 384 || recovery_cases != 384 || reset_cases != 48 || profiles != 6 ||
        recovery_retention_checks != 384 || drained_route_checks != 774 || stream_ticks != 1536 ||
        invalid_retention_checks != invalid_ticks+invalid_holds ||
        invalid_capture_ticks != invalid_ticks || invalid_phase_updates != invalid_dynamic_ticks ||
        invalid_frequency_updates == 0 || invalid_loads == 0 || invalid_auto_ticks == 0 ||
        invalid_external_ticks == 0 || pending_resets < 48 ||
        checks != 3*steps+4*resets+live_control_checks ||
        first_cells != 8'hff || second_cells != 8'hff ||
        retained_diagnostic_checks == 0 || restored_diagnostic_checks == 0 ||
        heating_ticks == 0 || cooling_ticks == 0 || target_lag_samples == 0)
      $fatal(1, "Incomplete M32 reserved-mode recovery coverage");
    for (int i = 0; i < 5; i++) if (state_hits[i] == 0) $fatal(1, "Missing valid cadence");
    if (state_hits[5] != 0 || state_hits[6] != 0 || state_hits[7] != invalid_ticks)
      $fatal(1, "Unexpected INVALID-state tick accounting");
    $display("FRP_M32_RESERVED_MODE_RECOVERY_TB: PASS profiles=%0d steps=%0d checks=%0d output_bits=2075 entry_cases=%0d recovery_cases=%0d reset_cases=%0d stream_ticks=%0d invalid_ticks=%0d invalid_holds=%0d invalid_dynamic_ticks=%0d invalid_phase_updates=%0d invalid_frequency_updates=%0d invalid_capture_ticks=%0d invalid_auto_ticks=%0d invalid_external_ticks=%0d invalid_loads=%0d invalid_retention_checks=%0d diagnostic_checks=%0d retained_diagnostic_checks=%0d restored_diagnostic_checks=%0d recovery_retention_checks=%0d drained_route_checks=%0d dynamic_ticks=%0d held_edges=%0d loads=%0d resets=%0d pending_resets=%0d clear_ticks=%0d clear_holds=%0d target_lag_samples=%0d state_feedback_samples=%0d switch_feedback_samples=%0d heating_ticks=%0d cooling_ticks=%0d first_legs=%0d second_legs=%0d first_cells=%0d second_cells=%0d live_control_checks=%0d free_ticks=%0d balance_ticks=%0d commit_ticks=%0d excite_ticks=%0d neutralize_ticks=%0d",
      profiles, steps, checks, entry_cases, recovery_cases, reset_cases, stream_ticks,
      invalid_ticks, invalid_holds, invalid_dynamic_ticks, invalid_phase_updates,
      invalid_frequency_updates, invalid_capture_ticks, invalid_auto_ticks, invalid_external_ticks,
      invalid_loads, invalid_retention_checks, diagnostic_checks, retained_diagnostic_checks,
      restored_diagnostic_checks, recovery_retention_checks, drained_route_checks,
      dynamic_ticks, held_edges, loads, resets, pending_resets, clear_ticks, clear_holds,
      target_lag_samples, state_feedback_samples, switch_feedback_samples,
      heating_ticks, cooling_ticks, first_legs, second_legs, first_cells, second_cells,
      live_control_checks, state_hits[0], state_hits[1], state_hits[2], state_hits[3], state_hits[4]);
    $finish;
  end

  initial begin
    #500000;
    $fatal(1, "FRP M32 reserved-mode recovery watchdog expired");
  end
endmodule
