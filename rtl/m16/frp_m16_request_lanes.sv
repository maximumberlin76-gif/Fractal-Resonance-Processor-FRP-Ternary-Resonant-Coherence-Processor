...
/*
    ...
    Purpose:
        Integrated request-lane arbitration module for the M16 core.

        This module matches the frp_m16_core.sv instance interface:
        ...
        It performs:
            ...
            - scheduler gate validation;
            - pending-route busy rejection;
            - same-state retention acceptance;
            - 0 -> +/-1 request acceptance when scheduler permits commit;
            - +/-1 -> 0 request acceptance when scheduler permits neutralize;
            - opposite-polarity request neutralization;
            - request event counting;
            - accepted / rejected lane masks;
            - accepted / rejected retained-element masks;
            - direct-transition request counting;
            - active-neutral routed request counting.

        It does not write retained state directly.
        It only emits arbitration outputs consumed by:
...
module frp_m16_request_lanes #(
    ...
    function automatic logic [CELLS - 1:0] element_mask_value(
        input int element_index
    );
        logic [CELLS - 1:0] mask_value;
        begin
            mask_value = '0;
            mask_value = ({{(CELLS - 1){1'b0}}, 1'b1} << element_index);
            element_mask_value = mask_value;
        end
    endfunction
    ...
    always_comb begin
        ...
        for (int domain_index = 0; domain_index < CELLS; domain_index = domain_index + 1) begin
            ...
            retained_value =
                packed_state_value(state_q, domain_index);

            target_value_q =
                packed_state_value(target_q, domain_index);

            pending_value_q =
                packed_state_value(pending_route_q, domain_index);
        ...
        for (int lane_index = 0; lane_index < REQUEST_LANES; lane_index = lane_index + 1) begin
            ...
            if (!invalid_index) begin
                retained_value =
                    packed_state_value(state_q, element_index_int);

                pending_value =
                    packed_state_value(pending_route_q, element_index_int);

                lane_element_mask =
                    element_mask_value(element_index_int);
            end
