...
module frp_m16_assertions #(
  ...
  generate
    for (state_cell = 0; state_cell < CELLS; state_cell++) begin : g_state_domain_assertions
      ...
      assert property (
        frp_is_valid_ternary(state_out[LSB +: STATE_BITS])
      ) else $error("FRP M16 assertion failed: state_out contains reserved ternary encoding");

      assert property (
        frp_is_valid_ternary(pending_route_out[LSB +: STATE_BITS])
  ...
  ) else $error("FRP M16 assertion failed: scheduler counters do not sum to ticks_recorded");

  assert property (
    (scheduler_mode_q == FRP_MODE_FREE) |-> (scheduler_state_q == FRP_SCHED_FREE)
  ...
  ) else $error("FRP M16 assertion failed: accepted_changes exceeds REQUEST_LANES");
  ...
  ) else $error("FRP M16 assertion failed: capacity_remaining exceeds REQUEST_LANES");
  ...
  ) else $error("FRP M16 assertion failed: switch_load_numerator must equal accepted_changes");
  ...
  ) else $error("FRP M16 assertion failed: actual_direct_events must remain zero");

  assert property (
    reserved_state_events == '0
  ) else $error("FRP M16 assertion failed: reserved_state_events must remain zero");

  assert property (
    queue_overflow_events == '0
  ) else $error("FRP M16 assertion failed: queue_overflow_events must remain zero");

  assert property (
    prevented_direct_events >= requested_direct_events
  ) else $error("FRP M16 assertion failed: prevented_direct_events < requested_direct_events");

  assert property (
    neutral_routed_events >= prevented_direct_events
  ) else $error("FRP M16 assertion failed: neutral_routed_events < prevented_direct_events");
  ...
  ) else $error("FRP M16 assertion failed: state-domain invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_SCHEDULER_COUNTS_VALID]
  ) else $error("FRP M16 assertion failed: scheduler-count invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_REQUEST_LANE_ORDER_VALID]
  ) else $error("FRP M16 assertion failed: request-lane invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_PENDING_POLARITY_VALID]
  ) else $error("FRP M16 assertion failed: pending-polarity invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_ACTIVE_NEUTRAL_VALID]
  ) else $error("FRP M16 assertion failed: active-neutral invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_TRANSITION_CAPACITY_VALID]
  ) else $error("FRP M16 assertion failed: transition-capacity invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_STATE_UPDATE_VALID]
  ) else $error("FRP M16 assertion failed: state-update invariant flag is false");

  assert property (
    invariant_flags[FRP_INV_NO_ACTUAL_DIRECT_EVENTS]
  ) else $error("FRP M16 assertion failed: no-actual-direct-events flag is false");

  assert property (
    invariant_flags[FRP_INV_NO_RESERVED_STATE]
  ) else $error("FRP M16 assertion failed: no-reserved-state flag is false");

  assert property (
    invariant_flags[FRP_INV_NO_QUEUE_OVERFLOW]
  ) else $error("FRP M16 assertion failed: no-queue-overflow flag is false");
