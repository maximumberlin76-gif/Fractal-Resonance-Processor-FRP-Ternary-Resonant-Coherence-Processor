#!/usr/bin/env python3
"""Binary clock-gated baseline for the FRP architecture comparison suite."""
...
ARCHITECTURE_SELF_TEST_SCHEMA = (
    "frp.benchmark.binary_clock_gated_reference.self_test.v1"
)
...
SEMANTIC_TO_BINARY_STATE = {
    NEGATIVE_TARGET: BINARY_ZERO,
    POSITIVE_TARGET: BINARY_ONE,
}


class BinaryClockGatedError(ValueError):
    """Raised when the binary clock-gated reference receives invalid input."""
...
class BinaryClockGatedReference:
    """Deterministic binary baseline with state-register clock gating."""
    ...
    def _target_state(
        ...
        try:
            return SEMANTIC_TO_BINARY_STATE[
                semantic_target
            ]
    ...
    def execute_command(
        ...
        *,
        ...
        if (
            semantic_target
            not in SEMANTIC_TO_BINARY_STATE
        ):
            raise BinaryClockGatedError(
        ...
        if state_clock_active:
            ...
            events["encoded_bit_toggles"] = 1
            events["register_write_bits"] = 1

            self.logical_state_changes += 1
            self.encoded_bit_toggles += 1
            self.direct_binary_switches += 1
        ...
        *,
        ...
        digest_payload = {
            ...
            "execution_profile": {
                ...
                "active_state_bits_per_command": 1,
...
def execute_binary_clock_gated_reference(
    workload_package: Mapping[str, Any],
) -> dict[str, Any]:
    """Execute one workload on the binary clock-gated baseline."""
    ...
    if (
        ...
        raise BinaryClockGatedError(
            ...
            "requires transaction_serial"
        )

    processor = (
        BinaryClockGatedReference(
            num_cells=profile[
                "num_cells"
            ]
        )
...
def _raises_clock_gated_error(
    callback: Any,
) -> bool:
    try:
        callback()
    except BinaryClockGatedError:
        return True
...
def run_self_test() -> dict[str, Any]:
    ...
    checks = {
        ...
        "gated_cycles_include_noop_and_cooldown": (
            specific[
                "gated_cycles"
            ]
            == expected_cycles
        ...
        "clocked_state_bits_match_writes": (
            raw[
                "clocked_state_bits"
            ]
            == raw[
        ...
        "clock_gate_metric_matches_common_metric": (
            math.isclose(
                specific[
                    "clock_gate_active_fraction"
                ],
        ...
        "binary_change_toggle_equivalence": (
            raw[
                "logical_state_changes"
            ]
            == raw[
        ...
        "register_write_matches_changes": (
            raw[
                "register_write_bits"
            ]
            == raw[
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Execute the Binary Clock-Gated "
            "Reference for the FRP Comparative "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Run binary clock-gated reference "
            "self-tests."
        ),
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.output == "json":
            ...
            if args.self_test:
                print(
                    "FRP COMPARATIVE BINARY "
                    "CLOCK-GATED REFERENCE SELF TEST"
                )
                ...
                print(
                    "active_clock_fraction="
                    f"{metrics['active_clock_fraction']}"
                )

                print(
                    "gated_cycles="
                    f"{specific['gated_cycles']}"
                )
        ...
        raise SystemExit(
            "binary clock-gated reference error: "
            f"{exc}"
        ) from exc
