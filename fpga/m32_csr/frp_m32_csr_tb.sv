...
// FRP M32 CSR integration testbench.
//
...
// All 59 FPGA core outputs are compared after each rising clock edge.
...
//
// Build from the repository root with Verilator 5.020:
// env verilator --binary --sv --timing --assert -Wall -Wno-fatal \
//   -CFLAGS "-std=c++20 -O0" --output-split 999999999 \
//   --top-module frp_m32_csr_tb --Mdir /tmp/frp-m32-csr-tb \
//   -Irtl/m22 -Irtl/m31 -Irtl/m32 -Ifpga/m32 -Ifpga/m32_csr \
//   fpga/m32_csr/frp_m32_csr_tb.sv
// /tmp/frp-m32-csr-tb/Vfrp_m32_csr_tb

`ifndef FRP_M32_CSR_TB_SV
`define FRP_M32_CSR_TB_SV

`timescale 1ns / 1ps
`include "frp_m32_csr_top.sv"
module frp_m32_csr_tb;
  import frp_m31_pkg::*;
  ...
  initial begin
    ...
    // One-shot external requests and both opposite routes with paused clocks.
    stage(0, 0, 1); wr(0, 1);
    ...
    ->inspect_request;
    @inspect_complete;
    $display("FRP_M32_CSR_TB: PASS checks=%0d ticks=%0d rejected_transfers=%0d core_outputs=59", checks, ticks, errors);
    $finish;
  end

  initial begin #2000000; $fatal(1,"timeout"); end
endmodule : frp_m32_csr_tb
