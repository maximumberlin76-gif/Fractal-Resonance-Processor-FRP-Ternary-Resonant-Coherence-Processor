...
// FRP M32 continuous CSR traffic, reset, and active-zero routing testbench.
// Author: Alchimist
//
// Run from the repository root with Verilator --binary --timing --assert,
// --top-module frp_m32_csr_stream_tb and include directories rtl/m22,
// rtl/m31, rtl/m32, fpga/m32, fpga/m32_csr.
// All observations use the nine external CSR ports. The scoreboard models
// register storage and scheduler accounting independently of the RTL core.
// Transactions occupy consecutive clock cycles; valid remains asserted
// across transfers unless an explicit idle or reset scenario changes it.
// Three fixed xorshift32 seeds make mixed traffic reproducible.

`ifndef FRP_M32_CSR_STREAM_TB_SV
`define FRP_M32_CSR_STREAM_TB_SV
`timescale 1ns / 1ps
`include "frp_m32_csr_top.sv"

module frp_m32_csr_stream_tb;
  logic clk = 0;
  logic rst_n_async = 0;
  ...
  initial begin
    #2000000;
    $fatal(1, "CSR stream watchdog expired");
  end
  ...
  task automatic mixed_stream(input logic [31:0] seed);
    ...
    audit();
    $display("CSR_STREAM seed=%08h PASS", seed);
  endtask
  ...
  initial begin : run
    ...
    $display("FRP_M32_CSR_STREAM_TB: PASS checks=%0d transfers=%0d ticks=%0d errors=%0d routes=%0d resets=%0d longest_run=%0d",
             checks, transfers, total_ticks, errors, routes, resets, longest_run);
    $finish;
  end
endmodule : frp_m32_csr_stream_tb
`endif
