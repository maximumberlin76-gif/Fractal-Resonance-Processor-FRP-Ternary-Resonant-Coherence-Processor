...
// FRP M32 deterministic FPGA integration testbench.
//
// Scope: eight cells, two request lanes, canonical M31 sine ROM.
// All 59 core outputs are compared with a separate frp_m32_core instance.
// Reference reset release follows scenario expectations, independently of
// the DUT synchronizer. Direct checks cover reset latency, startup control
// blocking, phase-load priority, registered-target capture, retained active
// zero and pending polarity, scheduler counts, pause, and counter clear.
// Both 7/1 and 1/7 modes are exercised alongside free execution.
//
// Run from the repository root with include directories rtl/m31, rtl/m32,
// and fpga/m32; select frp_m32_fpga_tb as the simulation top.

`ifndef FRP_M32_FPGA_TB_SV
`define FRP_M32_FPGA_TB_SV

`timescale 1ns / 1ps
`include "frp_m32_fpga_top.sv"

module frp_m32_fpga_tb;
  import frp_m31_pkg::*;
  ...
  initial begin
    #100000;
    $fatal(1, "FRP M32 FPGA TB timeout");
  end
  ...
  initial begin
    ...
    cycle(1'b1);
    check_reset_values();
    $display("PASS: 59 outputs match standalone M32 across %0d samples", samples);
    $display("FRP M32 FPGA integration testbench PASS");
    $finish;
  end
endmodule : frp_m32_fpga_tb
